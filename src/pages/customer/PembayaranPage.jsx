...
const PembayaranPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const totalHarga =
        response.data.biaya_pengiriman + response.data.total_pembayaran;
  ...
  const handleSubmit = (e) => {
    ...
    if (parseInt(pointsUsed) > totalSetelahPoin) {
      toast.error(
        "Penggunaan poin tidak boleh melebihi total pembayaran setelah poin."
      );
      return;
    }
    ...
    storePembayaran(id, finalFormData)
      .then((res) => {
        ...
        setTimeout(() => {
          navigate(`/pembayaran/nota/${id}?pointsUsed=${pointsUsed}`);
        }, 2000);
      })
      .catch((err) => {
        console.log(err);
    ...
    const poinYangBisaDigunakan = totalPoinSebelumnya;
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-4 grid grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="text-blue text-lg">
                      Rp {biayaPengriman.toLocaleString("id-ID")}
                    </p>
                  </div>
                </div>
                <div className="mb-4">
                ...
                <div className="mb-4">
                  ...
                  <p className="text-blue text-lg">
                    Rp {totalSetelahPoin.toLocaleString("id-ID")}
                  </p>
                </div>
                <div className="mb-4">
                  <label
                ...
                <div className="mb-4">
                  ...
                  <Input
                    ...
                    value={pointsUsed}
                    max={Math.min(sisaPoin, totalProduk)}
                    onChange={(e) => {
                      const value = parseInt(e.target.value);
                      const maxPointsToUse =
                        sisaPoin >= totalProduk ? totalProduk : sisaPoin;
                      setPointsUsed(
                        value > maxPointsToUse ? maxPointsToUse : value
                      );
                    }}
                  />
                </div>
